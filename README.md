...
| **SEO 분석** | 페이지 유형별 분석 — 홈페이지(직접 관리)는 15개 온페이지 항목, 외부 프로필(위키·교보문고·유튜브)은 콘텐츠·노출 중심, 검색결과(구글·다음)는 '이후 관련 페이지 노출 여부' 평가. 구글은 스크래핑 봇 차단으로 인한 거짓 음성을 피하기 위해 **Google Custom Search API** 사용 (`GOOGLE_CSE_API_KEY`·`GOOGLE_CSE_CX` 환경변수 필요, 미설정 시 0점 대신 '측정 불가' 표시 — 발급 절차는 `deploy/README.md` 참고) |
